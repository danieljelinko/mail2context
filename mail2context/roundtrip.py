...
__all__ = ['check_attachment', 'check_content', 'check_quote_stripping', 'check_reply_chain',
           'check_thread', 'content_body', 'plain_part_fate']
...
def check_content(delivered: EmailMessage,
    ...
    "Problems in `delivered` against `sent`: lost text (HTML, and what extract_text reads), lost colour, monospace body."
    ...
    # The plain part is not ours through Proton (regenerated on send, dropped on receipt), so
    # assert on the rendition the agent will actually read, whichever part that turns out to be.
    if lost := find_lost_chunks(src_sent, extract_text(delivered, strip_quotes=False)):
        problems.append(f'what extract_text reads lost text that was sent: {lost}')
...
def plain_part_fate(delivered: EmailMessage, sent: EmailMessage) -> str:
    "How the provider treated the text/plain part: 'as sent', 'regenerated by provider' or 'dropped by provider'."
    plain = delivered.get_body(('plain',))
    if plain is None: return 'dropped by provider'
    same = _squash(plain.get_content()) == _squash(sent.get_body(('plain',)).get_content())
    return 'as sent' if same else 'regenerated by provider'
