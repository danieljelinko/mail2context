...
from mail2context.render import html_to_text
...
def test_check_content_reports_what_the_agent_would_read_when_the_plain_part_was_gutted(make_rich_pair):
    # Given a delivered copy whose plain part — the rendition extract_text prefers — was replaced
    sent, delivered = make_rich_pair()
    ...
    # Then the loss is reported in terms of what extract_text reads, naming the missing runs
    assert any('extract_text' in p and 'cellule-A1' in p for p in problems)


def test_check_content_accepts_a_plain_part_the_provider_regenerated_from_the_html(make_rich_pair):
    # Given a delivered copy whose plain part is the provider's own flattening of the HTML, as
    # Proton does on send: same words, no Markdown markers
    sent, delivered = make_rich_pair()
    delivered.get_body(('plain',)).set_content(
        html_to_text(sent.get_body(('html',)).get_content(), strip_quotes=False))

    # When we check it
    problems = check_content(delivered, sent, accent_color=BLUE)

    # Then nothing is wrong: no text was lost, only markup the reader never needed
    assert problems == []


def test_check_content_accepts_a_delivered_copy_whose_plain_part_the_provider_dropped(make_rich_pair):
    # Given a delivered copy reduced to its HTML part, as Proton stores every alternative message
    sent, _ = make_rich_pair()
    html_only = EmailMessage(); html_only['Subject'] = sent['Subject']
    html_only.set_content(sent.get_body(('html',)).get_content(), subtype='html')

    # When we check it
    problems = check_content(html_only, sent, accent_color=BLUE)

    # Then nothing is wrong: the HTML is intact and it is what extract_text will read
    assert problems == []
...
def test_plain_part_fate_names_each_of_the_three_things_a_provider_can_do(make_rich_pair):
    # Given a faithful copy, a copy whose plain part was re-rendered, and one with no plain part
    sent, faithful = make_rich_pair()
    _, regenerated = make_rich_pair()
    regenerated.get_body(('plain',)).set_content('Rapport de fidélité du contenu, sans balises')
    html_only = EmailMessage(); html_only.set_content('<p>x</p>', subtype='html')

    # When we ask what happened to the plain part
    # Then each case is named
    assert plain_part_fate(faithful, sent) == 'as sent'
    assert plain_part_fate(regenerated, sent) == 'regenerated by provider'
    assert plain_part_fate(html_only, sent) == 'dropped by provider'
